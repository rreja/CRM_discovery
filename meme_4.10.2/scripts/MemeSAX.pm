...
my $RPT_OPT = 1; # repeat zero or more times
...
my $ST_IN_MODEL = [
  ...
  {ELE => 'wg', RPT => $RPT_OPT, CHARS => 1, END => \&_end_ele_wg},
  {ELE => 'ws', RPT => $RPT_OPT, CHARS => 1, END => \&_end_ele_ws},
  {ELE => 'endgaps', RPT => $RPT_OPT, CHARS => 1, END => \&_end_ele_endgaps},
...
sub _end_ele_regular_expression {
  my ($state) = @_;
  my $re = $state->_valid(undef, 'regular_expression', 'value', qr/^\s*([A-Zx\[\]]+)\s*$/);
