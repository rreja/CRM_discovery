...
sub seq_to_intern {
  ...
  for (my $line = 0; $line < scalar(@seqs); $line++) {
    my ($bitfields, $convert_errors) = seq_to_bitfield($seqs[$line], $alpha, \%nums, $any_bits, $line + 1);
...
#
sub seq_to_bitfield {
  my ($seq, $alpha, $nums, $any_bits, $lineno) = @_;
    ...
    if (!$inside_bracket) { #not inside a bracket expression
      ...
      if ($char eq ']') { #found the end of a bracket expression
        ...
        if ($bracket_value > 0) { #bracket had content so add it to the bitfields
          $bracket_value = ~$bracket_value & $any_bits if ($bracket_negate); #negated class
          push(@bitfields, $bracket_value);
        } else { #no valid characters within the bracket expression
          push(@nocontent, $bracket_start);
        }
      } elsif ($char eq '^' and $i==$bracket_start+1) { #negated character class
        $bracket_negate = 1;
      } elsif ($char =~ m/^([\Q$alpha\E])$/i) { #found a normal character
