...
MemeMenu.blurbs = {
  ...
  "meme": "<dfn>MEME</dfn> discovers novel, <em>ungapped</em> motifs (recurring, fixed-length patterns) in your <em>nucleotide or protein</em> sequences<span class=\"sample_output\"> (<a href=\"../doc/examples/meme_example_output_files/meme.html\">sample output</a> from <a href='http://meme-suite.org/meme-software/example-datasets/crp0.fna'>sequences</a>)</span>. <dfn>MEME</dfn> splits variable-length patterns into two or more separate motifs. <span class=\"manual_link\">See this <a href=\"../doc/meme.html?man_type=web\">Manual</a> for more information.</span> <footer><br><br>A motif is a sequence pattern that occurs repeatedly in a group of related protein or DNA sequences. MEME represents motifs as position-dependent letter-probability matrices which describe the probability of each possible letter at each position in the pattern.  Individual MEME motifs do not contain gaps. Patterns with variable-length gaps are split by MEME into two or more separate motifs. <br><br>MEME takes as input a group of DNA or protein sequences and outputs as many motifs as requested. MEME uses statistical modeling techniques to automatically choose the best width, number of occurrences, and description for each motif. <br><br>MEME on the web can take a second (control) set of input sequences and then discovers motifs that are enriched in the primary set relative to the control set.  This is called discriminative motif discovery.</footer>",

  "dreme": "<dfn>DREME</dfn> discovers <em>short</em>, <em>ungapped</em> motifs (recurring, fixed-length patterns) that are <em>relatively</em> enriched in your <em>nucleotide</em> sequences compared with shuffled sequences or your control sequences<span class=\"sample_output\"> (<a href=\"../doc/examples/dreme_example_output_files/dreme.html\">sample output</a> from <a href='http://meme-suite.org/meme-software/example-datasets/Klf1.fna'>sequences</a>)</span>.<span class=\"manual_link\"> See this <a href=\"../doc/dreme.html?man_type=web\">Manual</a> or this <a href=\"../doc/dreme-tutorial.html\">Tutorial</a> for more information.</span>  <footer><br><br>DREME (Discriminative Regular Expression Motif Elicitation) finds relatively short motifs (up to 8 positions) fast. The input to DREME is one or two sets of DNA sequences.  The control sequences should be approximately the same length as the primary sequences. If you do not provide a control set, the program shuffles the primary set to provide a control set. The program uses Fisher's Exact Test to determine significance of each motif found in the postive set as compared with its representation in the control set, using a significance threshold that may be set on the command line.<br><br>DREME achieves its high speed by restricting its search to regular expressions based on the IUPAC alphabet representing bases and ambiguous characters, and by using a heuristic estimate of generalised motifs' statistical significance.</footer>",

  "memechip": "<dfn>MEME-ChIP</dfn> performs <em>comprehensive motif analysis</em> (including motif discovery) on LARGE (50MB maximum) sets of <em>nucleotide</em> sequences such as those identified by ChIP-seq or CLIP-seq experiments<span class=\"sample_output\"> (<a href=\"../doc/examples/memechip_example_output_files/index.html\">sample output</a> from <a href='http://meme-suite.org/meme-software/example-datasets/Klf1.fna'>sequences</a>)</span>.<span class=\"manual_link\"> See this <a href=\"../doc/meme-chip.html?man_type=web\">Manual</a> for more information.</span>  <footer><br><br>MEME-ChIP can: <ol> <li>discover novel DNA-binding motifs (with MEME and DREME),</li> <li>determine which motifs are most centrally enriched (with CentriMo),</li> <li>analyze them for similarity to known binding motifs (with Tomtom), and</li> <li>automatically group significant motifs by similarity,</li><li>perform a motif spacing analysis (with SpaMo), and,</li><li>create a GFF file for viewing each motif's predicted sites in a genome browser.</li></ol>It is worth noting that MEME-ChIP is <b>not</b> a motif scanner, but the motifs discovered by it can be used by FIMO or MAST to scan for motif sites.</footer>",
  ...
  "fimo": "<dfn>FIMO</dfn> scans a nucleotide or protein sequence database for <em>individual matches</em> to each of the motifs you provide<span class=\"sample_output\"> (<a href=\"../doc/examples/fimo_example_output_files/fimo.html\">sample output</a> for <a href='http://meme-suite.org/meme-software/example-datasets/some_vertebrates.meme'>motifs</a> and <a href='http://meme-suite.org/meme-software/example-datasets/mm9_tss_500bp_sampled_1000.fna'>sequences</a>)</span>.<span class=\"manual_link\"> See this <a href=\"../doc/fimo.html?man_type=web\">Manual</a> for more information.</span>  <footer><br><br>The name FIMO stands for 'Find Individual Motif Occurrences'.  The program searches a database of DNA or protein sequences for occurrences of known motifs, treating each motif independently.  Motifs must be in <a href=meme-format.html>MEME Motif Format</a>. The web version of FIMO also allows you to type in motifs in additional formats. <br><br> You can define the statistical threshold (<i>p</i>-value) for motifs and whether FIMO scans just the given sequences or their reverse complements (where applicable), too.</footer>", 

  "mast": "<dfn>MAST</dfn> searches sequences for matches to a set of <em>nucleotide</em> or <em>protein</em> motifs, and sorts the sequences by the <em>best combined match</em> to all motifs<span class=\"sample_output\"> (<a href=\"../doc/examples/mast_example_output_files/mast.html\">sample output</a> for <a href='http://meme-suite.org/meme-software/example-datasets/adh.meme'>motifs</a> and <a href='http://meme-suite.org/meme-software/example-datasets/adh.faa'>sequences</a>).</span><span class='manual_link'> See this <a href=\"../doc/mast.html?man_type=web\">Manual</a> for more information.</span>  <footer><br><br>The name MAST stands for 'Motif Alignment and Search Tool'.  A motif is a sequence pattern that occurs repeatedly in a group of related protein or DNA sequences. Motifs are represented as position-dependent probability (or scoring) matrices that describe the probability (or score) of each possible letter at each position in the pattern. Individual motifs may not contain gaps. Patterns with variable-length gaps must be split into two or more separate motifs before being submitted as input to MAST.  <br><br>MAST takes as input a file containing the descriptions of one or more motifs and searches a sequence database that you select for sequences that match the motifs.  Motifs must be in <a href=meme-format.html>MEME Motif Format</a>. The web version of MAST also allows you to type in motifs in additional formats.  <br><br>MAST can search DNA or RNA sequences with DNA motifs, protein sequences with protein motifs, and protein sequences with DNA motifs.  You can control how (or if) MAST scans the reverse complement strand (if applicable).  You can also set thresholds for overall (combined match) significance and individual motif site significance.</footer>",
  ...
  "topics": [
    {
      ...
      "topics": [
        ...
        {
          "title": "EBI (Australia)",
          "info": "Go to the Australian MEME Suite site.",
          "url": "http://meme.ebi.edu.au",
          "absolute": true
        },
        {
          "title": "->View Current Load",
          "info": "View the current load at the Australian MEME Suite site.",
          "url": "http://meme.ebi.edu.au/opal2/dashboard?command=statistics",
          "absolute": true
        },
        {
