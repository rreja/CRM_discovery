...
static void print_meme_training_set_xml(
  ...
  fprintf(
    ...
    xmlify(dataset->datafile, buffer, true),
    dataset->n_samples
  );
  // Print alphabet
  if (dataset->dna) {
...
static void print_meme_model_xml(
  ...
  fprintf(
    ...
    xmlify(dataset->command, buffer, false)
  );
  fprintf(
    outfile,
    "<host>%s</host>\n"
  ...
  fprintf(
    ...
    "<prob>%g</prob>\n"
    "<spmap>%s</spmap>\n",
    model->min_nsites, model->max_nsites, dataset->wnsites, dataset->prob,
    xmlify(dataset->mapname, buffer, false)
  );
  fprintf(
    outfile,
    "<spfuzz>%g</spfuzz>\n"
    ...
    "<seqfrac>%8g</seqfrac>\n",
    dataset->map_scale,
    xmlify(dataset->priorname, buffer, false),
  ...
  if (dataset->plib_name) {
    fprintf(outfile, "%s", xmlify(dataset->plib_name, buffer, false));
  }
  fprintf(outfile, "</priors_file>\n");
  fprintf(
    outfile,
...
static void print_meme_contributing_sites_xml(
  ...
  buffer = str_create(10);
    ...
    for(i = 0; i < motif_summary->num_sites; i++) {
      ...
      fprintf(outfile, "<left_flank>%s</left_flank>\n<site>\n", xmlify(pre, buffer, false));
      int j = 0;
      for(j = 0; j < motif_summary->width; j++) {
        if (site[j] == '*') {
          fprintf( outfile, "<letter_ref letter_id=\"letter_star\"/>\n");
        } else {
