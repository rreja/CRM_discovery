...
)
{
  ...
  int kmer = 0;				// size of kmer to preserve if shuffling
  int n_copies = 1;			// number of copies to create if shuffling
  ...
  DO_STANDARD_COMMAND_LINE(1,
    ...
    DATA_OPTN(1, shuf, <k-let>, \t\tshuffle sequences preserving <k-let> frequencies, kmer = atoi(_OPTION_)); 
    DATA_OPTN(1, copies, <ncopies>, \tnumber of shuffled copies to make, n_copies = atoi(_OPTION_));
    USAGE(\n\tMeasure statistics of a FASTA file or (higher-order) shuffle its sequences.);
  )
  ...
  */
  ...
  while (read_sequence(data_file, &sample_name, &id, &seq, &length)) {
    ...
    if (kmer > 0) {
      ...
      char *shuf = NULL;
      Resize(shuf, length+1, char);
      shuf[length] = '\0';
      ushuffle1(seq, length, kmer);     
      int i_copy;
      for (i_copy=0; i_copy<n_copies; i_copy++) {
	// Create the new sample name.
	char *name = NULL;
	Resize(name, strlen(sample_name) + pad + 14, char);
	sprintf(name, "%s_shuf_%d", sample_name, i_copy);
	//printf(">%s\n%s\n", sample_name, seq);
