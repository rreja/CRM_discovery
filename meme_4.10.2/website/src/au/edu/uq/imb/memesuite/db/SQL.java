...
public final class SQL {
  ...
  public static final String SELECT_UNSORTED_CATEGORIES =
    ...
    "SELECT categoryId, c.name, count(l.id) FROM tblCategory AS c INNER JOIN tblListing AS l WHERE c.id = categoryId GROUP BY categoryId";
  ...
  public static final String SELECT_SEQUENCE_CATEGORIES =
    ...
    // "    SELECT DISTINCT listingId FROM tblSequenceFile\n" +
    // "    WHERE obsolete = 0 AND (alphabet & ?) != 0 AND (NOT ? OR maxLen < "+SML+")\n" +
    // "  )\n" +
    // ") ORDER BY name ASC";
    "SELECT d.id, d.name, cnt FROM \n" +
    "  (SELECT c.id, c.name, count(l.id) AS cnt FROM \n" +
    "    tblCategory AS c INNER JOIN tblListing AS l WHERE c.id = categoryId GROUP BY categoryId) AS d \n" +
    "  WHERE d.id IN ( \n" +
    "    SELECT DISTINCT categoryId FROM tblListing WHERE id IN ( \n" +
    "      SELECT DISTINCT listingId FROM tblSequenceFile \n" +
    "      WHERE obsolete = 0 AND (alphabet & ?) != 0 AND (NOT ? OR maxLen < "+SML+")\n" +
