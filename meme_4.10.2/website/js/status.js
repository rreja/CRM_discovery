...
function resize_preview() {
  var page, iframe_elems, iframe, rect, scroll;      
  page = (document.compatMode === "CSS1Compat") ? document.documentElement : document.body;
  iframe_elems = $("preview_container").getElementsByTagName("iframe");
...
function page_load() {
  resize_preview();
  if (/\b(pending|active|suspended)\b/.test($("message").className)) {
    job_status_timer = setInterval(query_status, job_status_interval);
  }
}
