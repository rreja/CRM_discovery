...
var SequenceInput = function(container, options) {
  ...
  this._xml_alph = (options.alphabets.RNA ? 1 : 0) + (options.alphabets.DNA ? 2 : 0) + (options.alphabets.PROTEIN ? 4 : 0);
  this._xml_short = (options.short_only ? 1 : 0);
  // make obvious DNA, Protein allowed / disallowed
  toggle_class(container.querySelector("div.alphainc.dna"), "disallowed", !options.alphabets.DNA);
  toggle_class(container.querySelector("div.alphainc.protein"), "disallowed", !options.alphabets.PROTEIN);
...
SequenceInput.prototype._load_versions = function(listing_id) {
  ...
  request.addEventListener("load", function(evt) {
    ...
    // add the other options
    all_v = versions.getElementsByTagName("version");
    for (i = 0; i < all_v.length; i++) {
      version = all_v[i];
      id = version.getAttribute("id");
      name = version.getAttribute("name");
      alphabets = version.getAttribute("alphabets"); // Bitset where RNA = 1, DNA = 2, PROTEIN = 4
