...
XmlMotifParser.prototype.process_meme_doc = function (doc) {
  ...
  var symbol_ids = [];
  var symbols = AlphUtil.letters(alphabet);
  for (i = 0; i < symbols.length; i++) {
    var id = symbol_map[symbols.charAt(i)];
    if (id == null) throw new MotifError("MEME XML missing value /MEME/training_set/alphabet/letter[@symbol='" + symbols.charAt(i) + "']/@id .");
    symbol_ids.push(id);
  }
  // get the strands
...
XmlMotifParser.prototype.process_dreme_doc = function (doc) {
  ...
  if (background_ele == null) throw new MotifError("DREME XML missing /dreme/model/background .");
  var alphabet_type = background_ele.getAttribute("type");
  if (alphabet_type == "dna") {
    alphabet = AlphType.DNA;
  } else {
    throw new MotifError("DREME XML contains an unrecognised alphabet type \"" + alphabet_type + "\".");
  }
  var alphabet_ids = ["A", "C", "G", "T"];
  this.rating = 3;
  // get the strands
