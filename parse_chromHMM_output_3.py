...
def process_file_4(output,inputdir,peakdir,fasta,tss):
    
    states = process_file_3(inputdir,output)
    ...
    in0 = open(os.path.join(output,segment_fname),"rt")
    ...
    in1 = open(os.path.join(output,emissions_fname),"rt")
    for line in in1:
        cols = line.rstrip().split("\t")
        if line.startswith("state"):
            ...
            continue
        state = cols[0]
        outdir = os.path.join(output,"E"+state)
        ...
        return_closest(os.path.join(outdir,sloped_original),tss,outdir,state)
        ...
        for vals in states[int(state)]:
            ...
            infile = return_filename(peakdir,vals+"*")
            sloped = return_filename(outdir,"*_segments.gff")
    
            inter = os.path.join(peak_dir,infile)
            pybedtools.BedTool(os.path.join(peakdir,infile)).intersect(os.path.join(outdir,sloped),u=True).saveas(inter)
            ...
            for line in open(a.sequence(fi=fasta).seqfn).readlines():
...
def return_closest(sloped,tss,outdir,state):
    ...
    pybedtools.BedTool(tss).closest(sloped,D="a").saveas(inter)
    in0 = open(inter,"rt")
    for line in in0:
        cols = line.rstrip().split("\t")
        if cols[9] == ".":
...
def run():
    ...
    process_file_4(options.output,options.input,options.peakdir,options.fasta,options.tss)
